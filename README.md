...
An interactive telephony IVR agent capable of conversational slot negotiation, customer identity verification, calendar booking, and automated SMS confirmation dispatch.

---

## Key Features

- **Voice-guided**: slot negotiation state machine handling colloquial time references
- **Caller**: phone number verification and client record matching
- **Real-time**: calendar slot reservation
- **Automated**: SMS booking confirmation trigger
- **Human**: operator fallback transfer command
...
```mermaid
flowchart TD
    Call([Inbound Phone Call]) --> IVR[Voice Scheduler State Machine]
    IVR --> Negotiate{Slot Negotiation}
    Negotiate -->|Agreed| Book[Calendar Reservation]
    Book --> SMS[Trigger SMS Confirmation]
    Negotiate -->|Operator Requested| Transfer[Transfer to Receptionist]
```
...
├── tests/
│   ├── __init__.py
│   └── test_voice_appointment.py   # Automated test suite
...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/erhatechnologiesai/voice-appointment-agent.git
...
|---|---|---|
| `POST` | `/voice-appointment` | Process voice dialogue state and reserve appointment |

### Example Request

```bash
curl -X POST http://127.0.0.1:8000/voice-appointment -H "Content-Type: application/json" -d '{"caller_phone": "+15550244", "spoken_input": "Can I book a dental cleaning for next Monday morning?"}'
```
...
```bash
python -m unittest tests/test_voice_appointment.py
```
